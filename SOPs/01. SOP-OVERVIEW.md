...
- Creates WIP commit: `chore: WIP - removed Christmas toggle component (#8) 🚧`
...
**Example commit:**
```
chore: remove Christmas theme toggle 🔧
...
**Types & Emojis:**
- `feat` ✨ - New features (use `Closes #issue`)
- `fix` 🐛 - Bug fixes (use `Fixes #issue`)
- `docs` 📝 - Documentation (use `Closes #issue`)
- `style` 💄 - UI/styling changes (use `Closes #issue`)
- `refactor` ♻️ - Code refactoring (use `Closes #issue`)
- `perf` ⚡️ - Performance improvements (use `Closes #issue`)
- `test` ✅ - Tests (use `Closes #issue`)
- `chore` 🔧 - Maintenance (use `Closes #issue`)
- `ci` 🚀 - CI/CD changes (use `Closes #issue`)

**Note:** The closing keyword goes in the footer to auto-close issues when merged.
...
3. **Release Please automatically:**
   ...
   - When you merge the Release PR → creates GitHub Release + tag
...
3. Creates WIP commit: `chore: WIP - <summary> (#issue) 🚧`
4. Optionally pushes to remote
5. Shows concise progress summary

**Example commit:**
```
chore: WIP - removed toggle component, need CSS cleanup (#8) 🚧
```
...
# Commit and push
/commit
# Creates: fix(mobile): prevent menu close issue 🐛
#
# Fixes #45
...
# Issue #45 auto-closes when merged
```
...
/start <#>                 → Start/resume issue work
/checkpoint                → Save WIP progress
/switch <#>                → Switch to different issue
/commit                    → Final commit (issue complete)
/push                      → Push branch + get PR link
```

**Workflow:** issue → start → build → checkpoint (optional) → commit → push → merge
